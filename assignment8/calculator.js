...
// keep track of whether or not "selectPrevious" is supported
var threeKeyMode = false;
...
$(document).keypress(function(event) {
	if (event.key == "a") {
		...
		clickSelectedItem();
	} else if (threeKeyMode && event.key == "q") {
		selectPrevious();
	} 
		...
		if (threeKeyMode) {
			threeKeyMode = false;
			$("#switchMode").html("Mode: 2-key");
		} else {
			threeKeyMode = true;
			$("#switchMode").html("Mode: 3-key");
		}
	}
})
